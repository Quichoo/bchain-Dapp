...
import CarList from '../data/ebike';
...
function Rent() {
    ...
    useEffect(()=>{
        ...
        const getCarsArray = async() => {
            const carsArray1 = await contract.getCars();
            const x = [...carsArray1];
            setCarArray(x);
        }
    ...
    return (
        <FadeIn>
            <Styled.Primary sx={style.rent_text} text='Vechicles' variant='h4'/>
            <Box sx={style.search}>
                <Box>
                    <TextField id='outlined-basic' color={secondaryColor} label='Search Vechicle' variant='outlined' onChange={(e) => setSearch(e.target.value)}
                        defaultValue={search}
                        value={search}
                        InputProps={{
                            endAdornment: (
