...
function Payment() {
    ...
    useEffect(()=>{
        ...
        const getNumCars = async() => {
            const num = await contract.getNumOfCars();
            const formattedNum = num.toNumber();
            setNumCars(formattedNum);
        }
        ...
        <Grid2 container spacing={1}>
                ...
                <Grid2 item xs={12} sm={12} md={6} lg={4} xl={4} >
                    <Card sx={style.payment_container}>
                        <Stack direction='column' alignItems='center' spacing={1}>
                            ...
                            <Box>
                                <Styled.Secondary sx={[style.payment_textAlignment]} text='Total Cars Rented' variant='h6' />
                            </Box>
                        </Stack>
                    </Card>
                </Grid2>
                <Grid2 item xs={12} sm={12} md={6} lg={4} xl={4} >
                    <Card sx={style.payment_container}>
                        <Stack direction='column' alignItems='center' spacing={1}>
                            <Box>
                            <CarRentalIcon sx={[style.payment_textAlignment, style.payment_icon]} />  
                            </Box>
                            <Box>
                                <Typography sx={[style.payment_textAlignment]} variant='h4'>
                                    {numCars}
                            ...
                            <Box>
                                <Styled.Secondary sx={[style.payment_textAlignment]} text='Total Cars' variant='h6' />
                            </Box>
                        </Stack>
                    </Card>
                </Grid2>
